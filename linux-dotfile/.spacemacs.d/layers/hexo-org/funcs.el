...
(defun hexo-org/hexo-publish (commit-msg)
...
(defun hexo-org/hexo-org-add-read-more ()
  "add <!--more-->"
  (interactive)
  (insert "#+BEGIN_HTML\n<!--more-->\n#+END_HTML"))

(defun hexo-org/hexo-org-new-open-post (post-name)
...
(defun hexo-org/word-count-for-chinese ()
  "「較精確地」統計中 / 日 / 英文字數。
- 文章中的註解不算在字數內。
...
(defun hexo-org/hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(hexo-org//hotspots-sources))))

(defun hexo-org//hotspots-sources ()
  "Construct the helm sources for my hotspots"
  `((name . "cowlog's center")
    (candidates . (
                   ...
                   ("Open Github" . (lambda() (browse-url "https://github.com/fewtime")))
                   ("Open Blog" . (lambda() (browse-url "http://www.cowlog.com")))))
    (candidate-number-limit)
    (action . (("Open" . (lambda (x) (funcall x)))))))

(defun hexo-org/start-erc ()
  "Connect to IRC"
  (interactive)
...
(defun hexo-org//notify (title message)
...
(defun hexo-org//org-archive-tasks (prefix)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun hexo-org/org-archive-all-tasks ()
  (interactive)
  (hexo-org//org-archive-tasks "DONE")
  (hexo-org//org-archive-tasks "CANCELLED")
  (hexo-org//org-archive-tasks "FIXED")
  )
